...
class Credentials(db.Model):
	...
	def get_username_by_name(cls,username):
		cred_username = db.GqlQuery("SELECT * FROM Credentials WHERE username = :user",user=str(username)).get()
...
class Post(db.Model):
	...
	def register_post(cls,subject,content,author,num_likes):
		post = Post(subject=subject,content=content,author=author,num_likes=num_likes)
		return post
...
class Comment(db.Model):
	...
	def get_all_post_comments(cls,post_ID):
		comments = db.GqlQuery("SELECT * FROM Comment WHERE post_id = :post_ID ORDER BY created DESC",post_ID=post_ID)
		return comments
