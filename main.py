    ...
    def post(self):
        ...
        if not have_error:
            ...
            self.render("signup.html", error_username=error_username, 
            	error_password=error_password, error_valid=error_valid,error_email=error_email)
...
class WelcomeHandler(Handler):
    ...
    def get(self):
        ...
        username = d_func.Credentials.get_username_by_ID(
...
class NewPostHandler(Handler):
	...
	def post(self):
		...
		if not s_func.read_secure_cookie(self) == "":
			if subject and content:
				...
				self.render(
					'newpost.html', user=curr_user, subject=subject, content=content, error=error)
		else:
...
class Posted(Handler):
	...
	def post(self, post_id):
		...
		if not comment == "" and not curr_user == "":
			store_comment = d_func.Comment.add_comment(
                post_id, comment, curr_user)
			store_comment.put()
			error = ""
		else:
		...
		comments = d_func.Comment.get_all_post_comments(post_id)
		self.render("posted.html", user=curr_user, post=post, post_id=post_id,
                    author_post=author_post, comments=comments, error=error)
...
class EditPostHandler(Handler):
	...
	def get(self, post_id):
		...
		if d_func.Post.check_author_post(curr_user, post_id):
			self.render('newpost.html', post_id=post_id, user=curr_user,
                        type_post="Edit Post", subject=subject, content=content, error=error)
		else:
			self.redirect("/blog")
	...
	def post(self, post_id):
		...
		if d_func.Post.check_author_post(curr_user, post_id):
			if subject and content:                                   
				...
				error = "You need to put valid subject and content"
				self.render('newpost.html', post_id=post_id, type_post="Edit Post",
                        	user=curr_user, subject=subject, content=content, error=error)
		else:
			self.redirect("/blog")
...
class DeletePostHandler(Handler):
	...
	def get(self, post_id):
		...
		if d_func.Post.check_author_post(curr_user, post_id):
			self.render("deletepost.html", type_post="Delete post",
                        user=curr_user, post=post, post_id=post_id, error=error)
		else:
			self.redirect("/blog")
...
class EditCommentHandler(Handler):
	...
	def get(self, post_id):
		...
		if d_func.Comment.check_author_comment(curr_user, c_id):
			self.render("edit_delete_comment.html", type_post="Edit comment",
                        user=curr_user, comment=comment, post_id=post_id)
		else:
			self.redirect("/blog")
	...
	def post(self, post_id):
		...
		if d_func.Comment.check_author_comment(curr_user, c_id):
			if content:
				...
				error = "No content"
				self.render("edit_delete_comment.html", type_post="Edit comment",
                        	user=curr_user, comment=comment, post_id=post_id, error=error)
		else:
			self.redirect("/blog")
...
class DeleteCommentHandler(Handler):
	...
	def get(self, post_id):
		...
		if d_func.Comment.check_author_comment(curr_user, c_id):
			self.render("edit_delete_comment.html", type_post="Delete comment",
                        user=curr_user, comment=comment, post_id=post_id)
		else:
			self.redirect("/blog")
...
class LogoutHandler(Handler):
    ...
    def get(self):
        ...
        self.response.headers.add_header(
            "Set-Cookie", 'user_id=%s;Path=/' % logout)
        self.redirect("/blog/login")
